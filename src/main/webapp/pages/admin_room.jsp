...
<head>
    ...
    <meta property="og:title" content="${user.getUsername()}">
            ...
            <div class="contentik">
                ...
                <div id="dle-content">
                    <article class="fiuurkrk" itemprop="mainEntity" itemscope="" itemtype="http://schema.org/Book">
                        <c:if test="${book_adding_success}">
                            <font color="green">Книга успешно создана!</font>
                        </c:if>
                        <c:if test="${author_adding_success}">
                            <font color="green">Автор успешно создан!</font>
                        </c:if>
                        <h1>
                            Admin
                        </h1>
                        <form action="bookAdding" method="post">
                            ...
                            <button class="btn btn-big" type="submit" name="submit"
                                    title="добавить книгу">добавить книгу
                            </button>
                        </form>
                        <form action="authorAdding" method="post">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                            <button class="btn btn-big" type="submit" name="submit"
                                    title="добавить автора">добавить автора
                            </button>
                        </form>
                        ...
                        <div class="mnimulaz">
                                ...
                                <h2>Список активных заказов:</h2><br>
                                <div class="col-mov-left-user">
                                    <ul class="mov-list">
                                <c:forEach items="${orders}" var="order">
                                        <li>
                                            <div class="mov-label">
                                                ${order.getUser().getUsername()} (${order.getUser().getSurname()} ${order.getUser().getName()}), email: ${order.getUser().getEmail()}
                                                    <a itemprop="url" href="book?book_id=${order.getBook().getId()}" class="mnimukkkea">
                                                            ${order.getBook().getName()}
                                                    </a>
                                            </div>
                                            <div class="mov-desc">
                                                <form action="changeStatus" method="post" style="float: right; margin-left: 300px">
                                                статус:
                                                <select required name="status">
                                                    <option <c:if test="${order.getStatus()=='забронирована'}">selected</c:if> value="забронирована">забронирована</option>
                                                    <option <c:if test="${order.getStatus()=='выдана'}">selected</c:if> value="выдана">выдана</option>
                                                    <option <c:if test="${order.getStatus()=='задолженность'}">selected</c:if> value="задолженность">задолженность</option>
                                                    <option <c:if test="${order.getStatus()=='получена'}">selected</c:if> value="получена">получена</option>
                                                </select>

                                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                                    <input type="hidden" name="order_id" value="${order.getId()}">
                                                    <button class="btn btn-big" type="submit" name="submit"
                                                            title="сохранить статус">сохранить статус
                                                    </button>
                                                </form>
                                            </div>
