                                ...
                                <c:forEach items="${orders}" var="order">
                                        <li>
                                            ...
                                            <div class="mov-desc">
                                                ...
                                                статус: <c:if test="${order.getStatus()=='задолженность'}">
                                                <font color="red">задолженность!</font>
                                            </c:if>
                                                <c:if test="${order.getStatus()!='задолженность'}">
                                                    ${order.getStatus()}
                                                </c:if>
                                                <c:if test="${order.getStatus()=='забронирована'}">
                                                    <form action="cancelReservation" method="post" style="float: right; margin-left: 350px">
                                                        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                                        <input type="hidden" name="book_id" value="${order.getBook().getId()}">
                                                        <button class="btn btn-big" type="submit" name="submit"
                                                                title="отменить бронирование">отменить бронирование
                                                        </button>
                                                    </form>
                                                </c:if>
