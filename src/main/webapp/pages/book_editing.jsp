...
<head>
    ...
    <title>${book.getName()} редактирование</title>
            ...
            <div class="contentik">
                ...
                <div class="speedbar">
                    <div class="over"><span id="dle-speedbar"><span itemscope=""
                        ...
                        <a href="catalog?genre_id=${book.getGenre().getId()}" itemprop="url"><span itemprop="title">${book.getGenre().getGenre()}</span></a></span> » <a href="book?book_id=${book.getId()}" itemprop="url">${book.getName()}</a></span> » Редактирование
                    ...
                    <article class="fiuurkrk" itemprop="mainEntity" itemscope="" itemtype="http://schema.org/Book">
                        ...
                        <form action="editBook" method="post">
                            ...
                            <div class="combo_field">
                                <input type="text" placeholder="Название книги" class="wide" name="name"
                                       value="${book.getName()}" required>
                            </div>
                            <div class="combo_field">
                                <input type="text" placeholder="Ссылка на фотографию" class="wide" name="image_link"
                                       value="${book.getImageLink()}">
                            </div>
                            <div class="combo_field">
                                <input type="text" placeholder="Год" class="wide" name="year" value="${book.getYear()}">
                            </div>
                            <div class="combo_field">
                                <select id="genres" required class="wide" name="genre_id">
                                    <option <c:if test="${book.getGenre().getId()==1}">selected</c:if> value="1">Любовные романы</option>
                                    <option <c:if test="${book.getGenre().getId()==2}">selected</c:if> value="2">Фантастика и фэнтези</option>
                                    <option <c:if test="${book.getGenre().getId()==3}">selected</c:if> value="3">Приключения</option>
                                    <option <c:if test="${book.getGenre().getId()==4}">selected</c:if> value="4">Детективы и триллеры</option>
                                    <option <c:if test="${book.getGenre().getId()==5}">selected</c:if> value="5">Наука, Образование</option>
                                    <option <c:if test="${book.getGenre().getId()==7}">selected</c:if> value="7">Проза</option>
                                    <option <c:if test="${book.getGenre().getId()==10}">selected</c:if> value="10">Религия и духовность</option>
                                    <option <c:if test="${book.getGenre().getId()==12}">selected</c:if> value="12">Старинная литература</option>
                                    <option <c:if test="${book.getGenre().getId()==13}">selected</c:if> value="13">Юмор</option>
                                    <option <c:if test="${book.getGenre().getId()==15}">selected</c:if> value="15">Поэзия, Драматургия</option>
                                    <option <c:if test="${book.getGenre().getId()==18}">selected</c:if> value="18">Ужасы и мистика</option>
                                    <option <c:if test="${book.getGenre().getId()==19}">selected</c:if> value="19">Сказки</option>
                                </select>
                            </div>

                            <div class="combo_field">
                                <input name="amount" type="number" value="${book.getAmount()}" required>
                            </div>
                            <div class="combo_field">
                                <textarea name="description" placeholder="Описание книги">${book.getDescription()}</textarea>
                            </div>


                            <button class="btn btn-big" type="submit" name="submit"
                                    title="Сохранить">Сохранить
                            </button>
                        </form>
